...
void inserir_na_lista(lista_linear_t* ll, int valor) {
    if (lista_cheia(ll)) {
        int* novo = realloc(ll->elementos, sizeof(int) * ll->capacidade * 2);
...
int busca_binaria(lista_linear_t* ll, int valor) {
    ...
    while (inicio <= fim) {
        ...
        if (valor == ll->elementos[meio]) return meio;
        if (ll->elementos[meio] < valor) inicio = meio + 1;
        else fim = meio - 1;
    }
    return -1;
}
...
int busca_linear(lista_linear_t* ll, int valor) {
    for (int index = 0; index < ll->tamanho; index++) {
        if (ll->elementos[index] == valor) return index;
    }
    return -1;
}
...
void inserir_na_posicao_da_lista(lista_linear_t* ll, int valor, int posicao) {
    ...
    if (!ll->ordenada) {
        inserir_na_lista_nao_ordenada(ll, ll->elementos[indice]);
...
void remover_na_posicao_da_lista(lista_linear_t* ll, int posicao) {
    if (posicao < 0 || posicao > tamanho_lista_linear(ll) - 1) return;

    if (ll->ordenada) {
        for (int i = posicao; i < ll->tamanho - 1; i++) {
            ll->elementos[i] = ll->elementos[i + 1];
        }
    } else {
        ll->elementos[posicao] = ll->elementos[ll->tamanho - 1];
    }
    ll->tamanho--;
}
